...
def doSuitAttack(attack):
    ...
    if groupStatus == ATK_TGT_SINGLE:
        ...
        for t in target:
            toon = t['toon']
            toonHprTrack.append(
                Sequence(Func(toon.headsUp, battle, MovieUtil.PNT3_ZERO),
                         Func(toon.loop, 'neutral')))
    ...
    if battle.isSuitLured(suit):
        ...
        waitTrack = Sequence(Wait(resetTrack.getDuration()),
...
def getResetTrack(suit, battle):
    ...
    walkTrack = Sequence(
        ...
        ActorInterval(suit,
                      ...
                      endTime=1e-05), Func(suit.loop, 'neutral'))
...
def getAllyToonsDodgeParallel(target):
    ...
    toonTracks.append(
        Sequence(ActorInterval(toon, sidestepAnim), Func(toon.loop,
                                                         'neutral')))
    toonTracks.append(
    ...
    if anim == 1:
        ...
        track = Sequence(
            ...
            LerpScaleInterval(prop,
                              scaleUpTime,
                              scaleUpPoint,
                              startScale=startScale), Wait(remainDelay),
...
def getPropAppearTrack(prop,
                       ...
                       startScale=Point3(0.01),
                       poseExtraArgs=None):
    propTrack = Sequence(Wait(appearDelay),
...
def getThrowTrack(object, target, duration=1.0, parent=render,
                  gravity=-32.144):
    values = {}
...
def doClipOnTie(attack):
    ...
    toonTrack = getToonTrack(attack, damageDelay, ['conked'], dodgeDelay,
                             ['sidestep'])
    throwSound = getSoundTrack('SA_powertie_throw.mp3',
                               delay=throwDelay + 1,
                               node=suit)
...
def doPoundKey(attack):
    ...
    BattleParticles.setEffectTexture(particleEffect,
                                     'poundsign',
                                     color=Vec4(0, 0, 0, 1))
    suitTrack = getSuitTrack(attack)
    ...
    propTrack = Sequence(
        ...
        LerpScaleInterval(phone, 0.5, MovieUtil.PNT3_ONE,
                          MovieUtil.PNT3_NEARZERO), Wait(0.74),
        Func(receiver.wrtReparentTo, suit.getRightHand()),
        LerpPosHprInterval(receiver, 0.0001, Point3(-0.45, 0.48, -0.62),
                           VBase3(-87.47, -18.21, 7.82)), Wait(3.14),
...
def doShred(attack):
    ...
    paperPropTrack = getPropTrack(paper,
                                  suit.getRightHand(),
                                  paperPosPoints,
                                  2.4,
                                  1e-05,
    ...
    shredderPropTrack = getPropTrack(shredder,
                                     suit.getLeftHand(),
                                     shredderPosPoints,
                                     1,
                                     3,
    ...
    toonTrack = getToonTrack(attack,
                             suitTrack.getDuration() - 1.1, ['conked'],
...
def doFillWithLead(attack):
    ...
    legsSmotherEffect = BattleParticles.createParticleEffect(
        file='fillWithLeadSmother')
    BattleParticles.setEffectTexture(sprayEffect,
                                     'roll-o-dex',
                                     color=Vec4(0, 0, 0, 1))
    BattleParticles.setEffectTexture(headSmotherEffect,
                                     'roll-o-dex',
                                     color=Vec4(0, 0, 0, 1))
    BattleParticles.setEffectTexture(torsoSmotherEffect,
                                     'roll-o-dex',
                                     color=Vec4(0, 0, 0, 1))
    BattleParticles.setEffectTexture(legsSmotherEffect,
                                     'roll-o-dex',
                                     color=Vec4(0, 0, 0, 1))
    suitTrack = getSuitTrack(attack)
    ...
    pencilPropTrack = getPropTrack(pencil,
                                   suit.getRightHand(),
                                   pencilPosPoints,
                                   0.7,
                                   3.2,
    ...
    sharpenerPropTrack = getPropTrack(sharpener,
                                      suit.getLeftHand(),
                                      sharpenerPosPoints,
                                      1.3,
                                      2.3,
    ...
    torsoEffectHeight = ToontownGlobals.torsoHeightDict[
        toon.style.torso] * bodyScale / 2 + legsHeight
...
def doFountainPen(attack):
    ...
    if dmg > 0:
        ...
        splashTrack = Sequence(
            Func(battle.movie.needRestoreRenderProp, splash), Wait(1.65),
...
def doRubOut(attack):
    ...
    def hideParts(parts):
        ...
        for partNum in xrange(0, parts.getNumPaths()):
            ...
            track.append(
                LerpFunctionInterval(nextPart.setAlphaScale,
                                     fromData=1,
                                     toData=0,
                                     duration=0.2))
...
def doWriteOff(attack):
    ...
    padPropTrack = getPropTrack(pad, suit.getLeftHand(), padPosPoints, 0.5,
                                2.57, Point3(1.89, 1.89, 1.89))
    missPoint = lambda checkmark=checkmark, toon=toon: __toonMissPoint(
        checkmark, toon)
    pencilPosPoints = [
...
def doRubberStamp(attack):
    ...
    if suitType == 'a':
        ...
        stampPosPoints = [Point3(-0.64, -0.17, -0.03), MovieUtil.PNT3_ZERO]
    padPropTrack = getPropTrack(pad, suit.getLeftHand(), padPosPoints, 1e-06,
                                3.2)
    missPoint = lambda cancelled=cancelled, toon=toon: __toonMissPoint(
        cancelled, toon)
    propTrack = Sequence(
        ...
        Func(cancelled.reparentTo, render), Func(cancelled.setScale, 0.6),
    ...
    soundTrack = getSoundTrack('SA_rubber_stamp.mp3',
                               delay=1.3,
                               duration=1.1,
                               node=suit)
...
def doRazzleDazzle(attack):
    ...
    if hitSuit:
        ...
        hitPoint = lambda particleEffect=particleEffect, toon=toon, suit=suit: __toonMissPoint(
...
def doSynergy(attack):
    ...
    toonTracks = getToonTracks(attack,
    ...
    synergySoundTrack = Sequence(
        Wait(0.9),
        SoundInterval(globalBattleSoundCache.getSound('SA_synergy.mp3'),
                      node=suit))
...
def doTeeOff(attack):
    ...
    clubPropTrack = getPropTrack(club, suit.getLeftHand(), clubPosPoints, 0.5,
                                 5.2, Point3(1.1, 1.1, 1.1))
    suitName = attack['suitName']
    if suitName == 'ym':
    ...
    ballPropTrack = Sequence(
        getPropAppearTrack(ball, suit, ballPosPoints, 1.7,
                           Point3(1.5, 1.5, 1.5)),
        Func(battle.movie.needRestoreRenderProp, ball),
        Func(ball.wrtReparentTo, render), Wait(2.15))
...
def doBrainStorm(attack):
    ...
    BattleParticles.setEffectTexture(snowEffect,
                                     'brainstorm-box',
                                     color=effectColor)
    BattleParticles.setEffectTexture(snowEffect2,
                                     'brainstorm-env',
                                     color=effectColor)
    BattleParticles.setEffectTexture(snowEffect3,
                                     'brainstorm-track',
                                     color=effectColor)
...
def doBuzzWord(attack):
    ...
    for i in xrange(0, 5):
        ...
        if random.random() > 0.5:
            BattleParticles.setEffectTexture(effect,
                                             texturesList[i],
...
def doCanned(attack):
    ...
    if dmg > 0:
        ...
        can2Point = Point3(hitPoint.getX(),
                           hitPoint.getY() + 6.4, hitPoint.getZ())
        ...
        hprTrack = Sequence(Wait(propDelay + suitDelay),
        ...
        hprTrack = Sequence(
            Wait(propDelay + suitDelay),
            LerpHprInterval(can, throwDuration, canHpr), Wait(0.4),
        ...
        soundTrack = getSoundTrack('SA_canned_tossup_only.mp3',
                                   delay=2.6,
                                   node=suit)
    canTrack = Sequence(Parallel(throwTrack, scaleTrack, hprTrack),
...
def doDownsize(attack):
    ...
    cloudPos = Point3(toonPos.getX(), toonPos.getY(),
                      toonPos.getZ() + toon.getHeight() * 0.55)
...
def doPinkSlip(attack):
    ...
    soundTrack = getSoundTrack('SA_pink_slip.mp3',
                               delay=2.9,
                               duration=1.1,
                               node=suit)
...
def doReOrg(attack):
    ...
    if dmg > 0:
        ...
        def getChestTrack(part, attackDelay=attackDelay):
            ...
            return Sequence(Wait(attackDelay),
                            LerpHprInterval(part, 1.1, VBase3(180, 0, 0)),
                            Wait(1.1), LerpHprInterval(part, 1.1,
                                                       part.getHpr()))
...
def doSacked(attack):
    ...
    if dmg > 0:
        ...
        sackTrack = Sequence(Parallel(sackAppearTrack, scaleTrack, hprTrack),
...
def doGlowerPower(attack):
    ...
    for i in xrange(0, 3):
        ...
        leftTrack.append(
            getPropAppearTrack(leftKnives[i],
                               suit,
                               leftPosPoints,
                               1e-06,
                               Point3(0.4, 0.4, 0.4),
        ...
        rightTrack.append(
            getPropAppearTrack(rightKnives[i],
                               suit,
                               rightPosPoints,
                               1e-06,
                               Point3(0.4, 0.4, 0.4),
...
def doHalfWindsor(attack):
    ...
    tiePropTrack = getPropAppearTrack(tie,
                                      suit.getRightHand(),
                                      posPoints,
                                      0.5,
                                      Point3(7, 7, 7),
...
def doHeadShrink(attack):
    ...
    for point in cloudPoints:
        circleTrack.append(
            LerpPosInterval(shrinkCloud, 0.14, point, other=battle))
    ...
    cloudTrack.append(circleTrack)
    cloudTrack.append(
        LerpFunctionInterval(shrinkCloud.setAlphaScale,
                             fromData=1,
                             toData=0,
                             duration=0.7))
    ...
    damageAnims.extend(getSplicedLerpAnims('cringe', 0.64, 1.0,
                                           startTime=0.85))
    ...
    if dmg > 0:
        ...
        soundTrack = Sequence(
            Wait(2.1), SoundInterval(shrinkSound, duration=2.1, node=suit),
            Wait(1.6), SoundInterval(growSound, node=suit))
...
def doRolodex(attack):
    ...
    suitTrack = getSuitTrack(attack)
    propTrack = getPropTrack(rollodex,
                             suit.getLeftHand(),
                             propPosPoints,
                             1e-06,
                             4.7,
...
def doPlayHardball(attack):
    ...
    suitTrack = getSuitTrack(attack)
    ballPosPoints = [
        Point3(0.04, 0.03, -0.31),
        VBase3(-1.152, 86.581, -76.784)
    ]
    propTrack = Sequence(
        getPropAppearTrack(ball,
                           suit.getRightHand(),
                           ballPosPoints,
                           0.8,
                           Point3(5, 5, 5),
    ...
    if dmg > 0:
        ...
        propTrack.append(
            LerpPosInterval(ball, 0.1, Point3(x, y + 8.5, z + 0.6)))
        propTrack.append(LerpPosInterval(ball, 0.1, Point3(x, y + 9, z + 0.2)))
        propTrack.append(Wait(0.4))
        soundTrack = getSoundTrack('SA_hardball_impact_only.mp3',
                                   delay=2.8,
                                   node=suit)
    else:
        ...
        propTrack.append(
            LerpPosInterval(ball, 0.1, Point3(x, y - 5.5, z + 0.6)))
...
def doPowerTie(attack):
    ...
    if dmg > 0:
        hitSound = getSoundTrack('SA_powertie_impact.mp3',
                                 delay=2.9,
                                 node=suit)
        return Parallel(suitTrack, toonTrack, tiePropTrack, throwSound,
                        hitSound)
...
def doDoubleTalk(attack):
    ...
    BattleParticles.setEffectTexture(particleEffect2,
                                     'doubletalk-good',
                                     color=Vec4(0, 1.0, 0.0, 1))
    suitType = getSuitBodyType(attack['suitName'])
    if suitType == 'a':
...
def doHotAir(attack):
    ...
    BattleParticles.setEffectTexture(flecksEffect,
                                     'roll-o-dex',
...
def doPickPocket(attack):
    ...
    billPropTrack = getPropTrack(bill,
                                 suit.getRightHand(),
                                 billPosPoints,
                                 0.6,
                                 0.55,
...
def doFilibuster(attack):
    ...
    BattleParticles.setEffectTexture(sprayEffect,
                                     'filibuster-cut',
                                     color=color)
    BattleParticles.setEffectTexture(sprayEffect2,
                                     'filibuster-fiscal',
                                     color=color)
    BattleParticles.setEffectTexture(sprayEffect3,
                                     'filibuster-impeach',
                                     color=color)
    BattleParticles.setEffectTexture(sprayEffect4,
                                     'filibuster-inc',
                                     color=color)
    partDelay = 1.3
    partDuration = 1.15
...
def doSchmooze(attack):
    ...
    for i in xrange(0, 4):
        ...
        BattleParticles.setEffectTexture(upperEffect,
                                         textureNames[i],
                                         color=Vec4(0, 0, 1, 1))
        BattleParticles.setEffectTexture(lowerEffect,
                                         textureNames[i],
                                         color=Vec4(0, 0, 1, 1))
    ...
    for i in xrange(0, 4):
        upperPartTracks.append(
            getPartTrack(upperEffects[i], partDelay + i * 0.65, 0.8,
                         [upperEffects[i], suit, 0]))
        lowerPartTracks.append(
            getPartTrack(lowerEffects[i], partDelay + i * 0.65 + 0.7, 1.0,
...
def doHangUp(attack):
    ...
    propTrack = Sequence(
        ...
        LerpPosHprInterval(receiver, 0.0001, Point3(-0.53, 0.21, -0.54),
                           VBase3(-99.49, -35.27, 1.84)), Wait(dialDuration),
...
def doRedTape(attack):
    ...
    propTrack.append(
        getPropThrowTrack(attack, tape, [hitPoint],
...
def doParadigmShift(attack):
    ...
    if hitAtleastOneToon == 1:
        soundTrack = getSoundTrack('SA_paradigm_shift.mp3',
                                   delay=2.1,
                                   node=suit)
...
def doPowerTrip(attack):
    ...
    def getPowerTrack(effect, suit=suit, battle=battle):
        partTrack = Sequence(
            ...
            Func(effect.start, suit), Wait(0.4),
            LerpPosInterval(effect, 1.0, Point3(0, 15, 0.4)),
            LerpFunctionInterval(effect.setAlphaScale,
                                 fromData=1,
                                 toData=0,
                                 duration=0.4), Func(effect.cleanup),
...
def doBounceCheck(attack):
    ...
    if hitSuit:
        ...
        checkPropTrack.append(
            LerpScaleInterval(check, 0.3, MovieUtil.PNT3_NEARZERO))
    checkPropTrack.append(Func(MovieUtil.removeProp, check))
    toonTrack = getToonTrack(attack, damageDelay, ['conked'], dodgeDelay,
                             ['sidestep'])
    soundTracks = Sequence(
        getSoundTrack('SA_pink_slip.mp3',
...
def doWatercooler(attack):
    ...
    if dmg > 0:
        ...
        splash.setScale(0.3)
        splashTrack = Sequence(
            Func(battle.movie.needRestoreRenderProp, splash), Wait(3.2),
            Func(prepSplash, splash, __toonFacePoint(toon)),
            ActorInterval(splash, 'splash-from-splat'),
            Func(MovieUtil.removeProp, splash),
            Func(battle.movie.clearRenderProp, splash))
    toonTrack = getToonTrack(attack,
                             suitTrack.getDuration() - 1.5, ['cringe'], 2.4,
                             ['sidestep'])
    soundTrack = Sequence(
        Wait(1.1),
        SoundInterval(
...
def doFired(attack):
    ...
    BattleParticles.setEffectTexture(flecksSmall,
                                     'roll-o-dex',
                                     color=Vec4(0.8, 0.8, 0.8, 1))
    ...
    damageAnims.extend(
        getSplicedLerpAnims('slip-forward', 0.31, 0.8, startTime=1.2))
    toonTrack = getToonTrack(attack,
                             damageDelay=1.5,
                             splicedDamageAnims=damageAnims,
                             dodgeDelay=0.3,
...
def doAudit(attack):
    ...
    BattleParticles.setEffectTexture(particleEffect3,
                                     'audit-three',
                                     color=Vec4(0, 0, 0, 1))
    particleEffect4 = BattleParticles.createParticleEffect('Calculate')
    BattleParticles.setEffectTexture(particleEffect4,
                                     'audit-four',
                                     color=Vec4(0, 0, 0, 1))
    particleEffect5 = BattleParticles.createParticleEffect('Calculate')
    BattleParticles.setEffectTexture(particleEffect5,
                                     'audit-mult',
                                     color=Vec4(0, 0, 0, 1))
    suitTrack = getSuitTrack(attack)
    ...
    calcPropTrack = getPropTrack(calculator,
                                 suit.getLeftHand(),
                                 calcPosPoints,
                                 1e-06,
                                 calcDuration,
...
def doCrunch(attack):
    ...
    for i in xrange(0, numOfNumbers):
        ...
        numberTrack = Sequence(
            Wait(0.9), LerpScaleInterval(next, 0.6, MovieUtil.PNT3_ONE),
...
def doLiquidate(attack):
    ...
    if dmg > 0:
        ...
        puddleTrack = Sequence(
            ...
            Wait(damageDelay - 0.7), Func(puddle.reparentTo, battle),
            Func(puddle.setPos, toon.getPos(battle)),
            LerpScaleInterval(puddle,
                              1.7,
                              Point3(1.7, 1.7, 1.7),
                              startScale=MovieUtil.PNT3_NEARZERO), Wait(3.2),
            LerpFunctionInterval(puddle.setAlphaScale,
                                 fromData=1,
                                 toData=0,
                                 duration=0.8),
...
def doMarketCrash(attack):
    ...
    sizeTrack = Sequence(
        Wait(propDelay + suitDelay + 0.2),
        LerpScaleInterval(paper, throwDuration, Point3(6, 6, 6)), Wait(0.95),
        LerpScaleInterval(paper, 0.4, MovieUtil.PNT3_NEARZERO))
    propTrack = Sequence(Parallel(paperTrack, spinTrack, sizeTrack),
    ...
    damageAnims.extend(
        getSplicedLerpAnims('slip-forward', 0.31, 0.95, startTime=1.2))
...
def doBite(attack):
    ...
    if dmg > 0:
        ...
        teethAppearTrack.append(
            LerpPosInterval(teeth,
                            0.4,
                            pos=Point3(x, y + 3.2, toonHeight * 0.7)))
        teethAppearTrack.append(
            LerpPosInterval(teeth,
                            0.3,
                            pos=Point3(x, y + 4.7, toonHeight * 0.5)))
        teethAppearTrack.append(Wait(0.2))
        teethAppearTrack.append(
            LerpPosInterval(teeth,
                            0.1,
                            pos=Point3(x, y - 0.2, toonHeight * 0.9)))
        teethAppearTrack.append(Wait(0.4))
        scaleTrack = Sequence(
            Wait(throwDelay),
            LerpScaleInterval(teeth, throwDuration, Point3(8, 8, 8)),
            Wait(0.9), LerpScaleInterval(teeth, 0.2, Point3(14, 14, 14)),
            Wait(1.2), LerpScaleInterval(teeth, 0.3, MovieUtil.PNT3_NEARZERO))
        hprTrack = Sequence(
            Wait(throwDelay), LerpHprInterval(teeth, 0.3, Point3(180, 0, 0)),
            Wait(0.2),
            LerpHprInterval(teeth,
                            0.4,
                            Point3(180, -35, 0),
                            startHpr=Point3(180, 0, 0)), Wait(0.6),
            LerpHprInterval(teeth,
                            0.1,
                            Point3(180, -75, 0),
    ...
    damageAnims = [['cringe', 0.01, 0.7, 1.2], ['conked', 0.01, 0.2, 2.1],
                   ['conked', 0.01, 3.2]]
...
def doChomp(attack):
    ...
    if dmg > 0:
        ...
        teethAppearTrack.append(
            LerpPosInterval(teeth,
                            0.1,
                            pos=Point3(x, y - 1.2, toonHeight * 0.7)))
        teethAppearTrack.append(
            LerpPosInterval(teeth,
                            0.1,
                            pos=Point3(x, y - 0.7, toonHeight * 0.4)))
        teethAppearTrack.append(Wait(0.4))
        scaleTrack = Sequence(
            Wait(throwDelay),
            LerpScaleInterval(teeth, throwDuration, Point3(6, 6, 6)),
            Wait(0.9), LerpScaleInterval(teeth, 0.2, Point3(10, 10, 10)),
            Wait(1.2), LerpScaleInterval(teeth, 0.3, MovieUtil.PNT3_NEARZERO))
        hprTrack = Sequence(
            Wait(throwDelay), LerpHprInterval(teeth, 0.3, Point3(180, 0, 0)),
            Wait(0.2),
        ...
        teethAppearTrack.append(
            LerpScaleInterval(teeth, 0.6, MovieUtil.PNT3_NEARZERO))
        hprTrack = Sequence(
            Wait(throwDelay), LerpHprInterval(teeth, 0.3, Point3(180, 0, 0)),
            Wait(0.5),
            LerpHprInterval(teeth,
        ...
        propTrack = Sequence(Parallel(teethAppearTrack, hprTrack, animTrack),
                             Func(MovieUtil.removeProp, teeth),
                             Func(battle.movie.clearRenderProp, teeth))
    damageAnims = [['cringe', 0.01, 0.7, 1.2], ['spit', 0.01, 2.95, 1.47],
                   ['spit', 0.01, 4.42, 0.07], ['spit', 0.08, 4.49, -0.07],
                   ['spit', 0.08, 4.42, 0.07], ['spit', 0.08, 4.49, -0.07],
                   ['spit', 0.08, 4.42, 0.07], ['spit', 0.08, 4.49, -0.07],
                   ['spit', 0.01, 4.42]]
...
def doWithdrawal(attack):
    ...
    suitTrack = getSuitAnimTrack(attack)
    partTrack = getPartTrack(particleEffect, 1e-05,
    ...
    if dmg > 0:
        ...
        colorTrack.append(Func(battle.movie.needRestoreColor))
        colorTrack.append(
            Parallel(changeColor(headParts), changeColor(torsoParts),
...
def doGuiltTrip(attack):
    ...
    def getPowerTrack(effect, suit=suit, battle=battle):
        partTrack = Sequence(
            Wait(0.7), Func(battle.movie.needRestoreParticleEffect, effect),
            Func(effect.start, suit), Wait(0.4),
            LerpPosInterval(effect, 1.0, Point3(0, 15, 0.4)),
            LerpFunctionInterval(effect.setAlphaScale,
                                 fromData=1,
...
def doRestrainingOrder(attack):
    ...
    if dmg > 0:
        ...
        cloudTrack = getPartTrack(restraintCloud, 3.5, 0.2,
...
def doPeckingOrder(attack):
    ...
    for i in xrange(0, numBirds):
        ...
        birdTrack = Sequence(Wait(throwDelay),
        ...
        scaleTrack = Sequence(Wait(throwDelay),
                              LerpScaleInterval(next, 0.15, Point3(9, 9, 9)))
        birdTracks.append(
            Sequence(Parallel(birdTrack, scaleTrack),
