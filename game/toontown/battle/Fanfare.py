...
def makePanel(toon, showToonName):
    ...
    if showToonName is 1:
        panel.avNameLabel = DirectLabel(parent=panel,
                                        relief=None,
...
def makeMessageBox(panel, message, messagePos, messageScale, wordwrap=100):
    panel.itemFrame = DirectFrame(parent=panel,
                                  relief=None,
                                  text=message,
                                  text_pos=messagePos,
...
def doFanfare(delay, toon, panel):
    ...
    seq1 = Parallel(
        Sequence(Wait(delay + 4.0999999999999996),
                 SoundInterval(drumroll, node=toon), Wait(0.25),
