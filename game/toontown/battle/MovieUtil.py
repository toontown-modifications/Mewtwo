...
def createSuitReviveTrack(suit, toon, battle, npcs=[]):
    ...
    for mtoon in battle.toons:
        toonMTrack.append(
            Sequence(Wait(1.0), ActorInterval(mtoon, 'duck'),
...
def createSuitDodgeMultitrack(tDodge, suit, leftSuits, rightSuits):
    ...
    suitTracks.append(
        Sequence(ActorInterval(suit, sidestepAnim), Func(suit.loop,
                                                         'neutral')))
...
def createToonDodgeMultitrack(tDodge, toon, leftToons, rightToons):
    ...
    if toonDodgeList is leftToons:
        ...
        for t in toonDodgeList:
            toonTracks.append(
                Sequence(ActorInterval(t, sidestepAnim),
                         Func(t.loop, 'neutral')))
...
def createSuitTeaseMultiTrack(suit, delay=0.01):
    suitTrack = Sequence(
        Wait(delay), ActorInterval(suit, 'victory', startTime=0.5,
                                   endTime=1.9), Func(suit.loop, 'neutral'))
...
def getToonTeleportOutInterval(toon):
    ...
    runTrack = Sequence(ActorInterval(toon, 'teleport', duration=dur),
                        Wait(T_HOLE_CLOSES), Func(toon.detachNode))
...
def getToonTeleportInInterval(toon):
    ...
    jumpTrack = Sequence(Wait(delay), Func(toon.reparentTo, render),
...
def getSuitRakeOffset(suit):
    ...
    if suitName == 'gh':
        ...
        notify.warning('getSuitRakeOffset(suit) - Unknown suit name: %s' %
                       suitName)
        return 0
...
def createSuitStunInterval(suit, before, after):
    ...
    return Sequence(Wait(before), Func(stars.reparentTo, head),
                    ...
                    Func(stars.loop, 'stun'), Wait(after),
