...
def __createFishingPoleMultiTrack(lure, dollar, dollarName):
    ...
    dollarTrack = Sequence(
        ...
        getSplicedLerpAnimsTrack(dollar,
                                 dollarName,
                                 0.69999999999999996,
                                 2.0,
                                 startTime=3),
    ...
    if sidestep == 0:
        ...
        tracks.append(
            Sequence(Wait(3.7000000000000002),
...
def __createMagnetMultiTrack(lure,
    ...
    if isSmallMagnet == 1:
        tracks.append(
            getSoundTrack('TL_small_magnet.mp3',
                          delay=0.69999999999999996,
                          node=toon))
    else:
        tracks.append(
            getSoundTrack('TL_large_magnet.mp3',
                          delay=0.69999999999999996,
                          node=toon))
    return tracks
...
def __lureSmallMagnet(lure, npcs=[]):
    ...
    scale = Point3(0.84999999999999998, 0.84999999999999998,
                   0.84999999999999998)
    return __createMagnetMultiTrack(lure,
                                    magnet,
                                    pos,
                                    hpr,
                                    scale,
...
def __createSuitDamageTrack(battle, suit, hp, lure, trapProp):
    ...
    if trapName == 'banana':
        ...
        rakeTrack = Sequence(
            Wait(0.5),
            LerpHprInterval(trapProp, 0.10000000000000001, upHpr,
                            startHpr=hpr), Wait(0.69999999999999996),
            LerpHprInterval(trapProp, 0.40000000000000002, hpr,
                            startHpr=upHpr),
            LerpHprInterval(trapProp,
                            0.14999999999999999,
                            bounce1Hpr,
                            startHpr=hpr),
            LerpHprInterval(trapProp,
            ...
            LerpHprInterval(trapProp,
                            0.050000000000000003,
                            hpr,
                            startHpr=bounce2Hpr), Wait(0.20000000000000001),
        ...
        moveTrack = Sequence(
            ...
            LerpPosInterval(trapProp,
                            0.80000000000000004,
                            slidePos,
                            other=battle), Wait(1.1000000000000001),
            LerpScaleInterval(trapProp, 1, Point3(0.01, 0.01, 0.01)))
        animTrack = ActorInterval(trapProp,
                                  'marbles',
        ...
        soundTrack = Sequence(
            Wait(0.69999999999999996),
            SoundInterval(globalBattleSoundCache.getSound('TL_quicksand.mp3'),
        ...
        soundTrack = Sequence(
            ...
            SoundInterval(globalBattleSoundCache.getSound('TL_trap_door.mp3'),
        ...
        damageTrack = Sequence(Wait(2.2999999999999998),
                               Func(suit.showHpText, -hp, openEnded=0),
                               Func(suit.updateHealthBar, hp))
        explosionSound = base.loadSfx(
            'phase_3.5/audio/sfx/ENC_cogfall_apart.mp3')
        soundTrack = Sequence(
            SoundInterval(globalBattleSoundCache.getSound('TL_dynamite.mp3'),
                          duration=2.0,
                          node=suit),
...
def __createSuitResetPosTrack(suit, battle):
    ...
    walkTrack = Sequence(
        ...
        ActorInterval(suit,
                      ...
                      endTime=0.0001), Func(suit.loop, 'neutral'))
...
def createIncomingTrainInterval(battle, suit, hp, lure, trapProp):
    ...
    materializeIval.append(
        LerpColorScaleInterval(train, TRAIN_MATERIALIZE_TIME,
                               Point4(1, 1, 1, 1)))
    for tunnel in tunnels:
        materializeIval.append(
...
def __createSlideshowMultiTrack(lure, npcs=[]):
    ...
    tracks.append(
        getSoundTrack('TL_presentation.mp3',
                      delay=2.2999999999999998,
                      node=toon))
    tracks.append(
        getSoundTrack('AA_drop_trigger_box.mp3',
