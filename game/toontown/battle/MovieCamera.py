...
def motionShot(x, y, z, h, p, r, duration, other=None, name='motionShot'):
    if other:
        posTrack = LerpPosInterval(camera,
                                   duration,
                                   pos=Point3(x, y, z),
                                   other=other)
        hprTrack = LerpHprInterval(base.camera,
                                   duration,
                                   hpr=Point3(h, p, r),
                                   other=other)
    else:
...
def suitCameraShakeShot(avatar, duration, shakeIntensity, quake=0):
    ...
    def shakeCameraTrack(intensity,
        ...
        vertShakeTrack = Sequence(
            ...
            Func(base.camera.setZ,
                 base.camera.getZ() + intensity / 2), Wait(shakeDuration / 2),
            Func(base.camera.setZ,
                 base.camera.getZ() - intensity), Wait(shakeDuration / 2),
            Func(base.camera.setZ,
                 base.camera.getZ() + intensity / 2))
        horizShakeTrack = Sequence(
            Wait(shakeWaitInterval - shakeDuration / 2),
            Func(base.camera.setY,
                 base.camera.getY() + intensity / 4), Wait(shakeDuration / 2),
            Func(base.camera.setY,
                 base.camera.getY() - intensity / 2), Wait(shakeDuration / 2),
            Func(base.camera.setY,
                 base.camera.getY() + intensity / 4), Wait(shakeDuration / 2),
            Func(base.camera.lookAt, Point3(0, 0, 0)))
        shakeTrack = Sequence()
...
def avatarCloseUpThreeQuarterLeftShot(avatar, duration):
    return heldRelativeShot(avatar, -5.2, 5.45,
                            avatar.getHeight() * 0.66, -131.5, 3.6, 0,
...
def avatarCloseUpThreeQuarterRightFollowShot(avatar, duration):
    ...
    track.append(
        heldRelativeShot(avatar, 5.2, 5.45,
                         avatar.getHeight() * 0.66, 131.5, 3.6, 0,
                         duration * 0.65))
    track.append(
...
def avatarLureTrainTrackShot(avatar, duration):
    return heldRelativeShot(avatar, 0, -7.5, 1 + avatar.getHeight(), 0, 0, 0,
                            duration, 'avatarLureTrainTrackShot')


def avatarBehindThreeQuarterRightShot(avatar, duration):
    return heldRelativeShot(avatar, 7.67, -8.52,
                            avatar.getHeight() * 0.66, 25, 7.5, 0, duration,
...
def avatarCloseUpThreeQuarterLeftFireShot(avatar, duration):
    return heldRelativeShot(avatar, -8.2, 8.45,
                            avatar.getHeight() * 0.66, -131.5, 3.6, 0,
                            duration, 'avatarCloseUpThreeQuarterLeftShot')
