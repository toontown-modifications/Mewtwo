...
class DistributedDoor(DistributedObject.DistributedObject, DelayDeletable):
    ...
    def getTriggerName(self):
        if (self.doorType == DoorTypes.INT_HQ) or (
            ...
            return 'door_trigger_' + str(self.block) + '_' + str(
                self.doorIndex)
        else:
    ...
    def doPostAnnounceGenerate(self):
        ...
        if self.doorType == DoorTypes.EXT_COGHQ and self.zoneId == ToontownGlobals.SellbotHQ:
            doorTrigger = self.getBuilding().find('**/%s' %
                                                  self.getTriggerName())
    ...
    def getBuilding(self):
        ...
        if not self.building:
            self.notify.warning(
                "getBuilding() returned None, trying second method.")
    ...
    def avatarEnqueueTrack(self, avatar, duration):
        ...
        otherNP = self.getDoorNodePath()
        walkLike = ActorInterval(avatar,
                                 'walk',
                                 startTime=1,
                                 duration=duration,
                                 endTime=0.0001)
        standHere = Sequence(
            LerpPosHprInterval(nodePath=avatar,
                               other=otherNP,
                               duration=duration,
                               pos=offset,
                               hpr=VBase3(0, 0, 0),
    ...
    def isDoorHit(self):
        ...
        isFacingForward = -168.0 < base.localAvatar.getH(
            self.getDoorNodePath()) < 168.0
    ...
    def enterOpening(self, ts):
        ...
        self.doorTrack = Parallel(SoundInterval(self.openSfx, node=rightDoor),
                                  Sequence(
                                      HprInterval(rightDoor,
                                                  VBase3(0, 0, 0),
                                                  other=otherNP), Wait(0.4),
                                      Func(rightDoor.show),
    ...
    def exitDoorEnterClosing(self, ts):
        ...
        if not leftDoor.isEmpty():
            ...
            self.finishDoorExitTrack()
            self.doorExitTrack = Sequence(LerpHprInterval(
                nodePath=leftDoor,
                duration=1.0,
                hpr=VBase3(0, 0, 0),
                startHpr=VBase3(h, 0, 0),
